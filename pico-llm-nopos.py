...
def train_one_model(model,
    ...
    for epoch in range(1, epochs + 1):
        ...
        torch.save(model, 'nopos_transformer.pth')
    ...
    plt.xlabel('Steps')
    plt.ylabel('Loss')
    plt.title('No Positional Embedding - Training Loss Over Steps')
    plt.legend()
    plt.grid(True)
    plt.xticks(epoch_ticks, epoch_labels)
    # plt.show()
    plt.savefig('no_embeddings_losses.png')
...
def main():
    ...
    block_size = context_size #args.block_size
